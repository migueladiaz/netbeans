...
import model.Nota;
import servicios.FiltroServicios;
...
public class FilterJson implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
      ...
      throws IOException, ServletException {
        ...
        FiltroServicios fs = new FiltroServicios();
        boolean error = false;
        MensajeHttp info = new MensajeHttp();
        
        try {
            String key = ((HttpServletRequest)request).getHeader("API_KEY");
            
            ApiKey k = fs.comprobarKey(key);
            
            if(k != null){
                if(fs.comprobarDatosKey(k) == false){
                    error = true;
                    ((HttpServletResponse)response).setStatus(401);
                    info.setMensaje("Se ha superado la cuota establecida");
                }
            }else{
                ((HttpServletResponse)response).setStatus(500);
                info.setMensaje("Ha ocurrido un error");
                error = true;
            }
